...
class TTS(RemdisModule):
    def __init__(self,
                 pub_exchanges=['tts', 'vap'],  # Added 'vap' to publish speech state events
        ...
        # End of speech buffer delay (seconds)
        self.end_speech_buffer = 1.5
    ...
    def send_loop(self):
        ...
        while True:
            ...
            if snd_iu['update_type'] == RemdisUpdateType.COMMIT:
                # Add a small buffer delay before signaling end of speech
                # This helps ensure all audio is actually played before we signal the end
                time.sleep(self.end_speech_buffer)
    ...
    def synthesis_loop(self):
        while True:
            ...
            # Signal the start of speech (only if we have real text to speak)
            if output_text != '':
                with self.speech_state_lock:
                    if not self.is_speaking:
                        self.is_speaking = True
                        self.send_speech_start_signal()
    ...
    def send_speech_start_signal(self):
        """Send a signal to indicate the start of speech"""
        try:
            vap_event = {'event': 'START_SPEECH', 'timestamp': time.time()}
            vap_iu = self.createIU(vap_event, 'vap', RemdisUpdateType.ADD)
            self.publish(vap_iu, 'vap')
            sys.stdout.write(f"[TTS {time.time():.3f}] Sent START_SPEECH VAP event\n")
        except Exception as e:
            sys.stderr.write(f"Error sending START_SPEECH: {e}\n")

    def send_speech_end_signal(self):
        """Send a signal to indicate the end of speech"""
        try:
            vap_event = {'event': 'TTS_COMMIT', 'timestamp': time.time()}
            vap_iu = self.createIU(vap_event, 'vap', RemdisUpdateType.ADD)
            self.publish(vap_iu, 'vap')
            sys.stdout.write(f"[TTS {time.time():.3f}] Sent TTS_COMMIT VAP event\n")
        except Exception as e:
            sys.stderr.write(f"Error sending TTS_COMMIT: {e}\n")
