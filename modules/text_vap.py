...
class TextVAP(RemdisModule):
    def __init__(self,
        ...
        self.is_revoke_pending = False  # Flag to prevent text resets before commit
        ...
        while True:
            try:
                ...
                if update_type == RemdisUpdateType.ADD:
                    ...
                    # Reset is_revoke_pending flag on new input
                    if new_text_segment:
                        self.is_revoke_pending = False
                    ...
                    if is_web_partial:
                        self.accumulated_text = new_text_segment
                    elif new_text_segment:
                        self.accumulated_text = (self.accumulated_text + " " + new_text_segment) if self.accumulated_text else new_text_segment
                    else:
                        continue
                    ...
                    producer = input_iu.get('producer', '')
                    is_from_asr = producer == 'ASR_Module' or 'asr-final' in input_iu.get('id', '')

                    self.log(f"Processing COMMIT signal: '{committed_text}' from {producer}")

                    # For empty ASR commits, track them but don't immediately trigger dialogue response
                    # Instead, let the timeout mechanism handle the final processing
                    if is_from_asr and not committed_text:
                        self.log(f"Received empty ASR commit. Continuing timeout monitoring for accumulated text: '{self.accumulated_text}'")
                        # Update timestamp to prevent premature timeout
                        self.last_asr_timestamp = current_time
                        continue

                    # Regular handling for non-empty commits or non-ASR commits
                    if not committed_text or committed_text == self.last_committed_text_from_input:
                        self.log(f"Skipping empty or duplicate frontend COMMIT.")
                        self.cancel_timeout_check()
                        continue

                    self.cancel_timeout_check()
                    self.log("Backend timeout cancelled due to frontend COMMIT.")

                    # Clear revoke pending flag
                    self.is_revoke_pending = False

                    # Use the actual accumulated text for the final commit if committed_text is empty
                    final_text = self.accumulated_text.strip() if not committed_text else committed_text
                    self.last_committed_text_from_input = final_text

                    self.log(f"Final text for COMMIT: '{final_text}'")
                    self.send_emotion_action_concept(final_text) # Send final state

                    if final_text: # Send VAP events
                        vap_commit_iu_body = {'event': 'ASR_COMMIT', 'text': final_text}
                        self.publish(self.createIU(vap_commit_iu_body, 'vap', RemdisUpdateType.ADD), 'vap')
                        self.log(f"Published VAP event: ASR_COMMIT (from frontend commit) with text: '{final_text}'")
                        vap_turn_iu_body = {'event': 'SYSTEM_TAKE_TURN'}
                        self.publish(self.createIU(vap_turn_iu_body, 'vap', RemdisUpdateType.ADD), 'vap')
                        self.log(f"Published VAP event: SYSTEM_TAKE_TURN (from frontend commit)")

                    self.reset_state_after_commit_signal(final_text) # Reset state

                elif update_type == RemdisUpdateType.REVOKE:
                    self.log("Processing REVOKE.")

                    # Set the revoke pending flag to prevent immediate text reset
                    self.is_revoke_pending = True

                    # Only clear if no commits are pending
                    if not self.is_in_commit_process():
                        self.accumulated_text = ""  # Clear the accumulated text
                        self.current_emotion = "normal"; self.current_action = "wait"; self.current_concept = ""
                        self.send_emotion_action_concept("");

                    self.last_committed_text_from_input = ""
                    self.cancel_timeout_check()
    ...
    def is_in_commit_process(self):
        """Check if we're in the process of committing (to prevent premature text reset)"""
        # Use timeout state as indicator of pending commit
        return self.is_timeout_mode
    ...
    def reset_state_after_commit_signal(self, final_text=""):
        ...
        self.is_revoke_pending = False  # Clear revoke pending flag
    ...
    def parse_and_update_state(self, llm_output_string):
        ...
        prev_concept = self.current_concept
        state_changed = False

        try:
            b_match = re.search(r'b:\s*(?:\d+_)?([a-zA-Z]+)', llm_output_string, re.IGNORECASE | re.MULTILINE)
            c_match = re.search(r'c:\s*([a-zA-Z0-9!?]+)', llm_output_string, re.IGNORECASE | re.MULTILINE)
            d_match = re.search(r'd:\s*(.+?)(?:\n|a:|b:|c:|$)', llm_output_string, re.IGNORECASE | re.MULTILINE)

            # Process emotion (b:)
            if b_match:
                new_emotion = b_match.group(1).lower().strip()
                if new_emotion != self.current_emotion:
                    self.current_emotion = new_emotion
                    state_changed = True
                    self.log(f"  Updated emotion: {new_emotion}")

            # Process backchannel (c:) - extract just the word without number prefix
            if c_match:
                backchannel_text = c_match.group(1).strip()
                # Remove any number prefix if present
                if "_" in backchannel_text:
                    backchannel_text = backchannel_text.split("_", 1)[1]

                # Extract just the text part for the action
                new_action = backchannel_text

                if new_action != self.current_action:
                    self.current_action = new_action
                    state_changed = True
                    self.log(f"  Updated action/backchannel: {new_action}")

            # Process concept (d:)
            if d_match:
                new_concept = d_match.group(1).strip().replace('"','').replace("'",'')
                if new_concept and new_concept != self.current_concept:
                    self.current_concept = new_concept
                    state_changed = True
                    self.log(f"  Updated concept: '{new_concept}'")
            ...
            self.log(f"Error parsing LLM: {e}\nString: '{llm_output_string}'")
    ...
    def timeout_monitor(self):
        ...
        while self.is_timeout_mode:
            ...
            if time_since_last_input >= self.max_silence_time:
                ...
                # Use current accumulated text for final text
                ...
                if final_text:
                    vap_commit_iu_body = {'event': 'ASR_COMMIT', 'text': final_text}
                    self.publish(self.createIU(vap_commit_iu_body, 'vap', RemdisUpdateType.ADD), 'vap')
                    self.log(f"Published VAP event: ASR_COMMIT (from backend timeout)")

                    vap_turn_iu_body = {'event': 'SYSTEM_TAKE_TURN'}
                    self.publish(self.createIU(vap_turn_iu_body, 'vap', RemdisUpdateType.ADD), 'vap')
                    self.log(f"Published VAP event: SYSTEM_TAKE_TURN (from backend timeout)")
                else:
                    self.log("Backend Timeout detected, but no text accumulated. No VAP events sent.")
